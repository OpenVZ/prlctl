...
*/
int PrlSrv::get_vm_list(PrlList<PrlVm *> &list, unsigned vmtype, unsigned list_flags)
{
	PRL_RESULT ret;
	...
	PrlHandle hJob(PrlSrv_GetVmListEx(m_hSrv, vmtype | list_flags));
...
int PrlSrv::update_vm_list(unsigned vmtype, unsigned flags)
{
	m_VmList.del();
	return get_vm_list(m_VmList, vmtype, flags);
}
...
void PrlSrv::append_slave_ifaces(PrlOutFormatter &f, const std::string& netId, bool detailed)
{
	...
	if (m_VmList.empty())
		update_vm_list(PVTF_CT|PVTF_VM, PGVLF_GET_STATE_INFO|PGVLF_GET_NET_INFO);
